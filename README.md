...
| 동작           | 조작                                                                 |
| -------------- | -------------------------------------------------------------------- |
| 카메라 회전    | 오른쪽 마우스 드래그 / 손가락 하나                                     |
| 카메라 이동    | 휠 클릭 드래그 / 손가락 둘                                     |
| 확대·축소      | 마우스 휠 / 핀치                                                     |
| 재난 실행      | 재난 선택 → 원하는 곳 좌클릭 / 재난 시작 / Enter                            |
| 발생 위치 변경 | 위치 지정 버튼 → 건물 또는 운동장 클릭, 또는 세부 설정에서 좌표 입력 |
| 직접 파괴 | 직접 파괴 카테고리 → 유형 선택 → 좌클릭·드래그 / 누르고 연속 적용 |
...
위치를 직접 정하면 얇은 원형 표시가 선택 지점을 보여주고, 재난을 시작하면 자동으로 사라져요. 직접 파괴는 기존 재난과 독립된 카테고리예요. 잡아 옮기기·물리·연소·부식·융해·변이·수압·중력 왜곡 중 하나를 고르고 범위와 강도를 조절할 수 있어요. 별도의 활성화 토글 없이 선택하면 바로 적용돼요. 좌클릭을 누르고 있으면 같은 위치에 효과가 누적되고, 드래그하면 표면을 따라 적용돼요. 지면 파기는 물리 파괴에 포함돼요. 일시정지 중에는 파괴가 멈춰요.
...
첨부 자료를 바탕으로 기숙사와 창의융합동 연결, 운동장과 테니스장 배치를 반영했어요. 정적인 주변 풍경과 연기 묶음은 그리기 요청을 합치고, 같은 지면을 반복해서 파는 연산은 생략해요. 굴착은 불규칙한 가장자리와 깊이를 가진 높이 지형으로 근사하며, 동굴·터널·지반 토사 유동은 구현하지 않아요.
...
직접 파괴의 **잡아 옮기기**를 선택하면 건물 부재나 잔해를 마우스로 잡아 움직일 수 있어요. 화면과 나란한 평면을 따라 물리적인 힘으로 끌며, 놓으면 중력이 적용돼요. 붙어 있는 부재를 잡으면 분리되고 지지하던 구조에도 영향을 줘요. 도구 변경, Esc, 일시정지, 창을 벗어날 때 잡기가 풀려요.

운석의 후속 화재는 충돌 후 주변에 남은 일부 지붕·목재·부속물에만 생겨요. 빠르게 날아가는 잔해는 발화 대상에서 제외해요. UFO는 물리 파괴와 변이를 사용해요. 불꽃은 높이·폭·기울기가 다르고, 연기는 어두운 회색 덩어리가 뭉쳐 올라가면서 생성 직후부터 조금씩 투명해져요. 사라질 때 크기를 줄이지 않아요.

설정의 **지면 굴착**을 끄면 새 구덩이가 생기지 않아요. 이미 생긴 구덩이는 캠퍼스 초기화로 복구해요. 굴착은 실제 아래로 파인 둥근 표면과 충돌 지형을 함께 변경하며 잔해도 그 안으로 떨어져요.


연소는 부재 하나에 여러 표면 발화점을 붙일 수 있어요. 불꽃은 넓은 밑부분과 갈라진 윗부분이 흔들리는 형태이고, 나무·지붕 등은 한 번 붙은 불이 오래 지속되며 열을 전해요. 같은 지점을 누르면 소모가 빨라져요. 융해는 연소와 별개로 달아오르며 주저앉는 상태예요. 화산의 용암은 분화구에서 세 갈래로 전진하며 보이는 흐름에 닿은 부재를 녹여요. 이는 간략한 경로 기반 표현이며 정밀한 유체 계산은 아니에요.

교내 도로와 코트 바닥을 파괴 가능한 판으로 나눴어요. 주변 산과 나무는 배경으로 유지해요. 운석은 깊은 중심부와 넓고 얕은 충격파 굴착을 함께 남겨, 넓은 범위가 손상돼도 전체가 같은 깊이로 파이지 않아요.
